...
export default function Home() {
  ...
  // 🔥 스릴러 톤(저음/느림) — 남성 강제는 불가하지만 체감 개선
  const ttsRate = 0.95;
  const ttsPitch = 0.62; // 더 저음
  const ttsVolume = 1.0;
  ...
  const speak = (text: string) => {
    ...
    try {
      ...
      // cancel 직후 바로 speak하면 씹히는 케이스 방지
  ...
  const checkWinAndMaybeEnd = (aliveState: Record<string, boolean>) => {
    ...
    const mafiaAliveCount = mafiaPlayers.filter(
      (p) => aliveState[p] !== false,
    ).length;
  ...
  const proceedAfterHandoff = () => {
    ...
    if (nightTurn === "MAFIA" && !mafiaAlive) {
      ...
      if (next === "SUMMARY") {
        ...
        speak(
          "밤 행동이 모두 종료되었습니다. 밤 결과를 적용하고 낮을 시작하세요.",
        );
      } else {
        goNightHandoff(next);
      }
  ...
  const setPreset = (m: "Day" | "Night") => {
    ...
    speak(
      m === "Day" ? "낮 토론 시간을 설정했습니다." : "밤 시간을 설정했습니다.",
    );
  };
  ...
  const executeVote = () => {
    ...
    if (!topVoted) {
      setLastEvent("⚠️ 집행 불가: 투표가 없r��나 1등 동점입니다.");
      ...
      <div className="max-w-md mx-auto px-5 py-6 relative">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            {/* TTS 에러 메시지 */}
            {ttsError && (
              <div className="text-[11px] text-amber-200/90 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2">
                {ttsError}
              </div>
            )}
            ...
            <button
              ...
              className="text-xs px-3 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10"
              title="브라우저 정책상 첫 클릭으로 음성을 언락해야 해요"
            >
              🔊 음성 시작
            </button>
          ...
          <div className={"mt-6 " + glowCard + " p-4"}>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-black/60 p-4 shadow-[0_0_30px_rgba(0,255,255,0.12)]">
              ...
              <div className="mt-3 text-xs text-white/50">
                * 이 화면에서는 아무 정보도 노출되지 않습니다. (역할 사망 시
                자동 스킵)
              </div>
            </div>
          </div>
        ...
        {phase === "NIGHT_POLICE" && (
          <div className={"mt-6 " + glowCard + " p-4"}>
            ...
            <p className="text-sm text-white/70 mt-1">
              조사 결과는{" "}
              <span className="text-white font-semibold">지금 화면에서만</span>{" "}
              보여집니다. (밤 종료 화면에서는 숨김)
            </p>
          ...
          <div className={"mt-6 " + glowCard + " p-4"}>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="mt-2 text-xs text-white/45">
                * 마피아 공격과 의사 치료는 동시에 적용됩니다. 같은 대상이면
                생존합니다.
              </div>
            </div>
          </div>
