...
export default function Home() {
  ...
  // ✅ 인원 선택 (요청: 기본 5 표시, 6명 이상부터 진행 가능)
  ...
  // ✅ 음성 안내(TTS) — 요청: “밤이 되었다”부터만
  const [ttsOn, setTtsOn] = useState(true);
  const [ttsReady, setTtsReady] = useState(false);
  const [ttsUnlocked, setTtsUnlocked] = useState(false);
  const [ttsVoice, setTtsVoice] = useState<SpeechSynthesisVoice | null>(null);
  const [ttsError, setTtsError] = useState<string>("");
  const [nightNarrationStarted, setNightNarrationStarted] = useState(false);
  ...
  // ✅ 안정형 speak — 요청 반영: “밤 시작” 전에는 말하지 않음
  const speak = (text: string) => {
    if (!ttsOn) return;
    if (typeof window === "undefined") return;

    // ✅ 밤 시작 전에는 음성 출력 금지
    if (!nightNarrationStarted && !isNightPhase(phase)) return;
    ...
    try {
      ...
      u.onend = () => setTtsError("");
      u.onerror = () =>
        setTtsError("TTS 재생 실패(브라우저/iframe 제한 또는 음소거). 새 탭에서 실행해보세요.");
  ...
  const setBgmKind = async (kind: "LOBBY" | "NIGHT" | "OFF") => {
    ...
    if (!audioOn || !audioUnlocked || kind === "OFF") {
      bgm.pause();
      return;
    }
  ...
  useEffect(() => {
    if (!audioUnlocked) return;
    void setBgmKind(isNightPhase(phase) ? "NIGHT" : "LOBBY");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, audioUnlocked, audioOn]);

  // ✅ role briefing 자동 반영
  const rolePlan = useMemo(() => recommendedRoles(playerCount), [playerCount]);

  // ✅ 살아있는 플레이어만
  const alivePlayers = useMemo(() => players.filter((p) => alive[p] !== false), [players, alive]);
  ...
  // ✅ 역할 생존 여부(스킵 로직)
  const mafiaAlive = useMemo(() => mafiaPlayers.some((p) => alive[p] !== false), [mafiaPlayers, alive]);
  const doctorAlive = useMemo(() => !!doctorPlayer && alive[doctorPlayer] !== false, [doctorPlayer, alive]);
  const policeAlive = useMemo(() => !!policePlayer && alive[policePlayer] !== false, [policePlayer, alive]);
  ...
  const checkWinAndMaybeEnd = (aliveState: Record<string, boolean>) => {
    ...
    const nonMafiaAliveCount = players.filter((p) => aliveState[p] !== false && assigned[p] !== "Mafia").length;
  ...
  const startGame = () => {
    ...
    setPhase("ROLES");
    // ✅ 요청 반영: 역할 확인/넘기는 동안 음성 없음 (speak 호출 X)
  };
  ...
  useEffect(() => {
    if (!running) return;
    if (secondsLeft <= 10 && secondsLeft > 0) setDangerPulse((p) => !p);
    else setDangerPulse(false);
  }, [secondsLeft, running]);

  // ✅ 타이머 마지막 10초 tick
  useEffect(() => {
    if (!running) return;
    if (secondsLeft <= 10 && secondsLeft > 0) playSfx("tick");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [secondsLeft, running]);
  ...
  const policeConfirm = () => {
    ...
    const msg = isMafia ? `🚨 조사 결과: ${policeTarget} = 마피아` : `🔎 조사 결과: ${policeTarget} = 마피아 아님`;
  ...
  const applyNightOutcome = () => {
    ...
    else if (pendingKill && pendingHeal && pendingKill === pendingHeal) msg += `${pendingKill} 님이 공격받았지만 치료로 살아남았습니다.`;
    else if (pendingKill && pendingHeal) msg += `${pendingKill} 님이 제거되었습니다. 그리고 ${pendingHeal} 님이 치료되었습니다.`;
  ...
  const resetAll = () => {
    ...
    setNightNarrationStarted(false);
    ...
    <div className="min-h-[100svh] text-white bg-[#07070a] relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto px-5 relative min-h-[100svh] flex flex-col">
        ...
        <div className="sticky top-0 z-20 pt-4 pb-3 bg-[#07070a]/80 backdrop-blur border-b border-white/10">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <button
                onClick={async () => {
                  ...
                  unlockTTS();
                  await unlockAudio();
                  if (audioOn) await setBgmKind("LOBBY");
                }}
                className="text-xs px-3 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10"
              ...
              <button
                onClick={() => {
                  ...
                  setAudioOn((v) => {
                    const next = !v;
                    if (!next) void setBgmKind("OFF"); // 끄면 BGM 즉시 정지
                    return next;
                  });
                }}
                className="text-xs px-3 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10"
              >
                {audioOn ? "🎵🔔 오디오 ON" : "🎵🔕 오디오 OFF"}
              </button>
            </div>
          </div>
          ...
          <p className="text-sm text-white/60 mt-1">인원 선택 → 역할 공개 → 낮 토론 → 투표 → 밤 행동 (사회자 없이)</p>
          ...
          {phase === "LOBBY" && (
            <div className="mt-6 space-y-4">
              <div className={glowCard + " p-4"}>
                ...
                <p className="text-xs text-white/50 mt-1">중앙 숫자에서 - / + 로 조절하세요 (권장: 6~12명)</p>
                ...
                <div className="mt-6 flex items-center justify-center gap-6">
                  <button
                    ...
                    onClick={() => {
                      playSfx("click");
                      setPlayerCount((c) => Math.max(5, c - 1));
                    }}
                    className={`w-14 h-14 text-2xl font-bold rounded-full border transition ${
                      locked ? "bg-white/5 text-white/30 border-white/10" : "bg-white/10 hover:bg-white/20 border-white/20"
                    }`}
                  >
                    -
                  </button>
                ...
                {playerCount < 6 && (
                  <div className="mt-4 text-xs text-amber-200/90 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2">
                    6명 이상부터 게임을 시작할 수 있어요.
                  </div>
                )}
          ...
          {phase === "ROLES" && (
            <div className={"mt-6 " + glowCard + " p-4"}>
              ...
              <p className="text-sm text-white/70 mt-1">
                휴대폰을 <span className="text-white font-semibold">{currentPlayer}</span> 님에게 넘겨주세요
              </p>

              {!confirmHandOff ? (
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/60 p-4 shadow-[0_0_30px_rgba(255,0,60,0.15)]">
                  <div className="text-white/70 text-sm">
                    화면을 몰래 보지 않게, <span className="text-white font-semibold">다음 사람이 직접</span> “넘겨받았음”을 눌러주세요.
                  </div>
